...
/* ─────────────────────────────────────────────
   Data fetch — top 10 by quantity sold
───────────────────────────────────────────── */
const fetchTopSelling = async () => {
  // salesdetail: transno, prodcode, quantity
  // Join product for description
  const { data, error } = await supabase
    .from('salesdetail')
    .select('prodcode, quantity, product(description, unit, record_status)');
  if (error) throw error;

  // Aggregate: sum quantity per prodcode
  const totals = {};
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;}
        ...
        .bar-info{width:160px;flex-shrink:0;}
        ...
        .bar-desc{font-size:12px;font-weight:600;color:var(--ink-2);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:155px;}
        ...
        .tbl-state{padding:56px;text-align:center;color:var(--muted);}
        ...
        .err-banner{margin:16px 24px;padding:12px 16px;border-radius:12px;background:#fef2f2;border:1px solid #fecaca;font-size:12px;color:#dc2626;font-weight:600;}

        @media(max-width:768px){
          .ts-topbar{padding:14px 20px;flex-wrap:wrap;height:auto;}
          .ts-body{padding:20px;}
          .summary-strip{grid-template-columns:1fr;}
          .bar-info{width:100px;}
        }
      `}</style>
      ...
      <div className="ts-root">
        ...
        <div className="ts-body">
          ...
          <div className="summary-strip">
            <div className="ss-card">
              ...
              <div>
                ...
                <div className="ss-val" style={{ fontSize: 16, paddingTop: 2 }}>{rows[0]?.description?.split(' ').slice(0,3).join(' ') || '—'}</div>
              </div>
            </div>
            <div className="ss-card">
            ...
            <div className="ss-card">
              ...
              <div>
                ...
                <div className="ss-val">{rows[0] ? `${rows[0].total_qty.toLocaleString()} ${rows[0].unit}` : '—'}</div>
              </div>
            </div>
          </div>
          ...
          {/* Chart / Table view */}
          {isLoading ? (
            <div className="chart-panel"><div className="tbl-state"><div className="loader" /><p>Loading top sellers…</p></div></div>
          ) : rows.length === 0 ? (
            <div className="chart-panel"><div className="tbl-state"><div style={{fontSize:28,marginBottom:8}}>📊</div><p>No sales data found.</p></div></div>
          ) : view === 'chart' ? (
            <div className="chart-panel">
              ...
              <div className="chart-area">
                {rows.map((r, idx) => {
                  ...
                  const wide  = pct > 30; // show qty inside bar if wide enough
                  return (
                    <div key={r.prodcode} className="bar-row" style={{ animationDelay: `${idx * 0.05}s` }}>
                      ...
                      <div className="bar-track">
                        <div
                          className="bar-fill"
                          style={{ width: `${pct}%`, background: BAR_COLORS[idx] ?? '#9ca3af' }}
                        >
