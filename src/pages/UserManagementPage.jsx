...
const Toast = ({ toasts }) => (
  <div style={{ position:'fixed', bottom:24, right:24, display:'flex', flexDirection:'column', gap:8, zIndex:9999, pointerEvents:'none' }}>
    {toasts.map(t => (
      <div key={t.id} style={{ padding:'10px 16px', borderRadius:8, background: t.type==='success' ? '#166534' : '#991b1b', color:'#fff', fontSize:13, fontWeight:500, fontFamily:"'DM Sans',sans-serif", boxShadow:'0 4px 16px rgba(0,0,0,0.18)', display:'flex', alignItems:'center', gap:8 }}>
...
const Spinner = ({ size = 12, color }) => (
  <span style={{ display:'inline-block', width:size, height:size, border:`1.5px solid ${color}40`, borderTop:`1.5px solid ${color}`, borderRadius:'50%', animation:'ump-spin 0.7s linear infinite', flexShrink:0 }} />
);

const SortIcon = ({ column, sortConfig }) => (
  <span style={{ fontSize:10, marginLeft:3, opacity: sortConfig.key===column ? 1 : 0.3, color: sortConfig.key===column ? '#111827' : 'inherit' }}>
...
const RoleBadge = ({ role }) => {
  ...
  return <span style={{ ...s, display:'inline-flex', alignItems:'center', padding:'2px 9px', borderRadius:999, fontSize:11, fontWeight:500, whiteSpace:'nowrap' }}>{role || 'USER'}</span>;
};

const StatusBadge = ({ status }) => {
  const isActive = status === 'ACTIVE';
  return (
    <span style={{ display:'inline-flex', alignItems:'center', gap:5, padding:'2px 9px', borderRadius:999, fontSize:11, fontWeight:500, background: isActive ? '#EAF3DE' : '#FCEBEB', color: isActive ? '#27500A' : '#791F1F', border: isActive ? '0.5px solid #97C459' : '0.5px solid #F09595', whiteSpace:'nowrap' }}>
      <span style={{ width:6, height:6, borderRadius:'50%', flexShrink:0, background: isActive ? '#639922' : '#E24B4A' }} />
      {status}
    </span>
  );
};
...
const UserManagementPage = () => {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      setFetchError(err.message); // PR-03
  ...
  const handleStatusChange = async (userId, newStatus) => {
    ...
    try {
      const { error } = await supabase.from('user').update({ record_status: newStatus }).eq('id', userId);
      if (error) throw error;
  ...
  return (
    <>
      <style>{`
        ...
        /* PR-03: error banner */
        .ump-error { display:flex; align-items:center; gap:10px; padding:12px 16px; border-radius:10px; background:#fef2f2; border:1px solid #fecaca; font-size:12px; font-weight:600; color:#dc2626; margin-bottom:16px; }
        .ump-retry { margin-left:auto; padding:5px 14px; border-radius:7px; border:1px solid #fecaca; background:#fff; color:#dc2626; font-family:'DM Sans',sans-serif; font-size:12px; font-weight:700; cursor:pointer; flex-shrink:0; transition:background .15s; }
        .ump-retry:hover { background:#fef2f2; }
        ...
        .ump-search { width:100%; padding:7px 10px 7px 32px; font-size:13px; font-family:'DM Sans',sans-serif; border:0.5px solid #e5e7eb; border-radius:8px; background:#fff; color:#111827; outline:none; transition:border-color 0.15s; }
        .ump-search:focus { border-color:#b91c1c; }
        .ump-select { padding:7px 10px; font-size:13px; font-family:'DM Sans',sans-serif; border:0.5px solid #e5e7eb; border-radius:8px; background:#fff; color:#111827; outline:none; cursor:pointer; }
        .ump-count { margin-left:auto; font-size:12px; color:#9ca3af; white-space:nowrap; }
        ...
        .ump-table thead th { background:#f9fafb; padding:9px 14px; font-size:11px; font-weight:500; color:#6b7280; text-transform:uppercase; letter-spacing:.06em; text-align:left; border-bottom:0.5px solid #e5e7eb; white-space:nowrap; cursor:pointer; user-select:none; }
        ...
        .ump-table td { padding:11px 14px; font-size:13px; color:#374151; vertical-align:middle; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
        ...
        .ump-btn { padding:5px 12px; font-size:12px; font-weight:500; border-radius:7px; cursor:pointer; font-family:'DM Sans',sans-serif; display:inline-flex; align-items:center; gap:5px; white-space:nowrap; transition:background 0.15s; }
        ...
        .ump-tip { visibility:hidden; opacity:0; pointer-events:none; position:absolute; bottom:calc(100% + 6px); left:50%; transform:translateX(-50%); background:#1f2937; color:#fff; font-size:11px; padding:5px 9px; border-radius:6px; white-space:nowrap; transition:opacity 0.15s; z-index:200; }
        ...
        .ump-empty-icon { font-size:28px; margin-bottom:8px; }
        ...
        @media(max-width:768px){
          ...
          .ump-select { width:100%; }
          .ump-count { margin-left:0; }
        }
      `}</style>
      ...
      <div className="ump-wrap">
        ...
        {/* PR-03: error state with retry */}
        {fetchError && !loading && (
          <div className="ump-error">
            <span>⚠</span>
            <span>Failed to load users: {fetchError}</span>
            <button className="ump-retry" onClick={fetchUsers}>Try again</button>
          </div>
        )}

        <div className="ump-stats">
          <div className="ump-stat"><div className="ump-stat-label">Total users</div><div className="ump-stat-val">{stats.total}</div></div>
          <div className="ump-stat"><div className="ump-stat-label">Active</div><div className="ump-stat-val g">{stats.active}</div></div>
          <div className="ump-stat"><div className="ump-stat-label">Inactive</div><div className="ump-stat-val r">{stats.inactive}</div></div>
          <div className="ump-stat"><div className="ump-stat-label">Admins</div><div className="ump-stat-val b">{stats.admins}</div></div>
        </div>
        ...
        <div className="ump-toolbar">
          <div className="ump-search-wrap">
            ...
            <input className="ump-search" type="text" placeholder="Search users…" value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        ...
        <div className="ump-table-wrap">
          <table className="ump-table">
            <colgroup>
              <col className="c-name"/><col className="c-email"/><col className="c-role"/>
              <col className="c-status"/><col className="c-joined"/><col className="c-actions"/>
            </colgroup>
            <thead>
              <tr>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="ump-empty">
                    <div className="ump-empty-icon">👥</div>
                    <div style={{fontWeight:600,color:'#374151',marginBottom:4}}>No users found</div>
                    ...
                    <tr key={user.id} className={isSA ? 'row-superadmin' : ''}>
                      <td>
                        <div className="ump-name-cell">
                          <div className="ump-avatar" style={{background:av.bg,color:av.color}}>{getInitials(user.full_name, user.email)}</div>
                      ...
                      <td>
                        {isSA ? (
                          <div className="ump-tip-wrap">
                            <button className="ump-btn ump-btn-protected" disabled>🔒 Protected</button>
                            <span className="ump-tip">SUPERADMIN accounts cannot be modified</span>
                          </div>
                        ) : isActive ? (
                          <button className="ump-btn ump-btn-deactivate" disabled={busy} onClick={() => handleStatusChange(user.id, 'INACTIVE')}>
                            {busy && <Spinner color="#791F1F" />}
                            Deactivate
                          </button>
                        ) : (
                          <button className="ump-btn ump-btn-activate" disabled={busy} onClick={() => handleStatusChange(user.id, 'ACTIVE')}>
